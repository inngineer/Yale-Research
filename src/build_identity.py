...
def main():
    print("=" * 60)
    print("🛠️  DYNAMIC IDENTITY LIBRARY BUILDER")
    print("=" * 60)
    
    # Load configuration
    print(f"\n📖 Loading configuration from {CONFIG_FILE}...")
    config = load_config()
    print(f"✅ Loaded profile: {config['identity_profile']}")
    
    # Generate C code
    print(f"\n⚙️  Generating {OUTPUT_C_FILE}...")
    c_code = generate_c_code(config)
    
    with open(OUTPUT_C_FILE, 'w') as f:
        f.write(c_code)
    print(f"✅ Generated {len(c_code)} bytes")
